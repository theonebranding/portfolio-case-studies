...
const Hero = () => {
  ...
  return (
    <section
      id="home"
      className="min-h-[60vh] sm:min-h-screen flex flex-col pt-24 sm:pt-20 pb-10 sm:pb-0 items-center justify-center relative overflow-hidden"
    >
      ...
      <div className="container mx-auto px-4 sm:px-6 z-10 text-center">
        ...
        <h1 className="rotating-text mt-4 sm:mt-8 h-12 sm:h-20 md:h-32 mb-8 sm:mb-12 overflow-hidden relative text-center">
          {/* Rotating Text */}
          <div className="absolute inset-0 flex items-center justify-center">
            {rotatingTexts.map((text, index) => (
              <span
                key={index}
                className={`rotating-text-item absolute inset-0 flex items-center justify-center text-base sm:text-2xl md:text-3xl lg:text-4xl font-primary font-bold gradient-text z-20 px-4 leading-tight text-center ${
          ...
          <div>
            {/* Gradients + Sparkles — hidden on mobile to avoid the giant empty block */}
            <div className="hidden sm:flex h-[40rem] w-full bg-brand-black flex-col items-center justify-center overflow-hidden rounded-md z-0 relative">
              ...
              />
              <div className="absolute inset-0 w-full h-full bg-brand-black [mask-image:radial-gradient(700px_250px_at_top,transparent_30%,white)]"></div>
            </div>
          </div>
        </h1>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-6">
          <Button asChild variant="primary" className="px-6 py-4 sm:px-8 sm:py-6 text-base sm:text-lg font-tertiary">
            <a href="#services" className="text-white">Explore Our Services</a>
          </Button>
          <Button
            asChild
            variant="glass"
